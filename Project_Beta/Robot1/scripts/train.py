...
#   python scripts/train.py --iteration 1             # Train for iteration 1
...
def train(
    ...
    experiments_dir = robot_dir / config['paths']['experiments']
    ...
    if manifest_path and manifest_path.exists():
        print(f"[Train] Loading data from manifest: {manifest_path}")
        data_dirs = load_data_dirs_from_manifest(manifest_path, training_data_dir)
    else:
        print(f"[Train] Scanning for data in: {training_data_dir}")
        data_dirs = find_data_directories(training_data_dir)
...
def main():
    ...
    if args.manifest:
        ...
        # Check for iteration manifest
        iteration_manifest = robot_dir / "experiments" / f"iteration_{args.iteration:03d}" / "dataset_manifest.json"
        if iteration_manifest.exists():
            manifest_path = iteration_manifest

    # Run training
    try:
        results = train(config, args.iteration, robot_dir, manifest_path)
